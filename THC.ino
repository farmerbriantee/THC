...
void initializeADC();
void initializeLCD();
void calibrateVoltageSetpoint();
void updateTorchHeight();
void updateDisplay();
void controlTHCOutput();
...
void setup()
{
    ...
    initializeLCD();
    calibrateVoltageSetpoint();

    // Before carrying on, we now reset those variables.
    for (i = 0; i < BUFSIZE; i++)
        values[i] = 0;
...
void calibrateVoltageSetpoint()
{
    ...
    unsigned long calibTotal = 0;
    unsigned int calibValues[10] = {0};
    int calibIndex = 0;
    unsigned long calibTarget = 0;
    ...
    while (ms < timelimit) {
        ...
        calibTotal = calibTotal + tmp - calibValues[calibIndex];
        calibValues[calibIndex] = tmp;
        calibTarget = calibTotal / 10;

        // Calculate the setpoint, based on min/max, and chop it to one decimal point.
        currentValue = MINSET + ((MAXSET - MINSET) * (calibTarget / 1023.0));
        currentValue = ((int)(currentValue * 10)) / 10.0;

        if (lastDisplayedValue != currentValue) {
            lastDisplayedValue = currentValue;
            timelimit = max(timelimit, ms + 2000);

            // Print the set number Pin A1
            lcd.setCursor(11, 0);
            lcd.print(lastDisplayedValue, 1);
        }

        calibIndex = (calibIndex + 1) % 10;
        ms = millis();
    }

    // Convert the voltage target back into an int, for ADC comparison, with the scale the plasma pin uses.
    target = lastDisplayedValue / SCALE;
}
...
void updateDisplay()
{
    ...
    {
        lcd.setCursor(8, 1);
        plasmaVoltage = (float)((disp / DISP) * SCALE);
        lcd.print(plasmaVoltage, 1);
...
void controlTHCOutput()
{
	...
	if (plasmaVoltage < 80 || plasmaVoltage > 150) digitalWrite(THC_PIN, THC_OFF);
}
